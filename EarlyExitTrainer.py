...
class ModelTrainer:
    ...
    def train_full_model(self, train_loader, epoch_count, validation_loader=None):
        ...
        lr = self.config.training_params[self.args.data][self.args.arch]["lr"]
        optim_name = self.config.training_params[self.args.data][self.args.arch]["optimizer"]
        if optim_name == "adam":
            ...
            optimizer = torch.optim.SGD(
                ...
                lr=lr,
                momentum=self.config.training_params[self.args.data][self.args.arch]["momentum"],
                weight_decay=self.config.training_params[self.args.data][self.args.arch]["weight_decay"],
                nesterov=False,
            )
            if self.config.training_params[self.args.data][self.args.arch]["lr_type"] == "multistep":
                lr_steps = self.config.training_params[self.args.data][self.args.arch]["decay_epochs"]
                gamma = self.config.training_params[self.args.data][self.args.arch]["decay_rate"]
        ...
        for epoch in range(epoch_count):
            ...
            self.progress_bar = tqdm(
                train_loader, desc=f"Epoch {epoch+1}", dynamic_ncols=True, leave=True
            )
            ...
            if validation_loader is not None:
                validation_loss, validation_accuracy = self.validate_classifier(
                    validation_loader
                )
    ...
    def train_classifiers(self, train_loader, epoch_count=1, validation_loader=None):
        ...
        for i in range(len(self.model.exit_modules)):
            ...
            for j in range(i):
                self.model.exit_modules[j].set_state(
                    TrainingState.TRAIN_CLASSIFIER_FORWARD
                )
    ...
    def train_exit_epoch(self, train_loader, lr, epoch, validation_loader=None):
        ...
        for exit_layer in self.model.exit_modules:
            trainable_params += list(
                filter(lambda p: p.requires_grad, exit_layer.exit_gate.parameters())
            )
        ...
        for i, (X, y) in enumerate(self.progress_bar):
            ...
            loss, ce_part, cost_part = self.gate_loss_function(
                y, y_hats, exit_confidences, self.model.costs
            )
            net_loss += loss.item()
            ...
            if validation_loader is not None and i % (len(train_loader) // 10) == 0:
                ...
                self.writer.add_scalar(f"Loss/alpha={self.alpha}/exit gates", loss, idx)
                self.writer.add_scalar(f"Loss Part 1/alpha={self.alpha}/exit gates", ce_part.item(), idx)
                self.writer.add_scalar(f"Loss Part 2/alpha={self.alpha}/exit gates", cost_part.item(), idx)

                self.writer.add_scalar(f"Accuracy/alpha={self.alpha}/exit gates", validation_accuracy, idx)
                self.writer.add_scalar(f"Time/alpha={self.alpha}/exit gates", validation_time, idx)
                self.writer.add_scalar(f"Exit Idx/alpha={self.alpha}/exit gates", exit_idx, idx)
    ...
    def train_exit_layers(self, train_loader, lr, epoch_count=1, validation_loader=None):
        ...
        for epoch in range(epoch_count):
            ...
            # set model state
            self.model.set_state(TrainingState.TRAIN_EXIT)
            _, validation_accuracy, validation_time, exit_idx = self.train_exit_epoch(
                train_loader, lr, epoch, validation_loader
            )
            validation_accuracies.append(validation_accuracy)
    ...
    def validate_classifier(self, validation_loader):
        ...
        with torch.no_grad():
            progress_bar = tqdm(
                validation_loader, desc=f"Validation", dynamic_ncols=True, leave=False
            )
    ...
    def validate_exit_gates(self, validation_loader):
        ...
        with torch.no_grad():
            ...
            for X_val, y_val in progress_bar:
                ...
                exits_taken_count = torch.tensor(
                    self.model.num_exits_per_module, device=self.device
                )
                weights = torch.arange(len(exits_taken_count), device=self.device) + 1

                weighted_avg = (torch.sum(exits_taken_count * weights)).item() / len(
                    X_val
                )
        ...
        return (
            total_accuracy / len(validation_loader),
            total_time / len(validation_loader),
            total_exit_index_taken / len(validation_loader),
        )
    ...
    def should_stop_early(self, validation_accuracy_list, num_acc_to_check=5):
        ...
        if all(
            validation_accuracy_list[i] > validation_accuracy_list[i + 1]
            for i in range(-num_acc_to_check, 0)
        ):
            return True
        return False
    ...
    def save_model(self, model_name):
        # save the full model
        assert (
            self.model.state != TrainingState.TRAIN_ARCH
        ), "You must use save_internal_model to save the internal model"
    ...
    def save_internal_model(self, model_name):
        ...
        assert (
            self.model.state == TrainingState.TRAIN_ARCH
        ), "Model must be in TRAIN_ARCH state to save internal model"
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
